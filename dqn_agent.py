...
class DQNAgent(base_agent.BaseAgent):
    ...
    def _sample_action(self, qs):
        ...
        for i in range(batch_size):
            if np.random.rand() < exp_prob:
                a[i] = torch.randint(num_actions, size=(1,))
            else:
    ...
    def _compute_q_loss(self, norm_obs, a, tar_vals):
        ...
        q_selected = torch.gather(q_values, dim=1, index=a)
        loss = torch.nn.functional.mse_loss(q_selected.squeeze(), tar_vals)
    ...
    def _sync_tar_model(self):
        ...
        tar_params = self._tar_model.parameters()
        # print("tar_params", tar_params)
        main_params = self._model.parameters()
        # print("main_params", main_params)

        for tar_param, main_param in zip(tar_params, main_params):
